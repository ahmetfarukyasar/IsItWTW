...
const Explore = () => {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      {loading && <div className="text-center mt-8 text-white text-xl sm:text-2xl">Loading...</div>}

      {!loading && movies.length === 0 && (
        <div className="text-center mt-8 text-white text-lg sm:text-xl">No movies found.</div>
      )}

      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 lg:gap-6 mt-6 sm:mt-8 justify-items-center">
        {movies.map((movie) => (
          <MovieCard
            key={movie.id}
            id={movie.id}
      ...
      {page < totalPages && !loading && (
        <div className="flex justify-center mt-8 sm:mt-12 mb-6 sm:mb-8">
          <button
            className="px-4 py-2 outline-2 rounded cursor-pointer bg-gray-600 hover:bg-gray-700 hover:text-white text-white"
